...
const emptyCssExtractionFile = path.join(
  ...
  'vanilla-extract-virtual-file.css',
);
...
export function pitch(this: webpack.LoaderContext<any>) {
  const { childCompiler, outputCss, identifiers } =
  ...
  const compiled = childCompiler.getCompiledSource(this) as Promise<{
    source: string;
  }>;
      ...
      const result = await processVanillaFile({
        ...
        serializeVirtualCssPath: async ({ fileName, source }) => {
          ...
          const request = loaderUtils.stringifyRequest(
            ...
            `${emptyCssExtractionFile}?${encodeURIComponent(
