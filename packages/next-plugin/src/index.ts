...
import { VanillaExtractWebpackPlugin } from './webpack-plugin';
...
type PluginOptions = ConstructorParameters<
  typeof VanillaExtractWebpackPlugin
>[0];
...
const getVanillaExtractCssLoaders = (
  ...
  loaders.push({
    loader: require.resolve('../virtual-loader'),
  });
...
export const createVanillaExtractPlugin = (
  ...
  return (nextConfig: NextConfig = {}): NextConfig => ({
    ...
    webpack(config: any, options: WebpackConfigContext) {
      ...
      // prettier-ignore
      const hasAppDir = !!resolvedNextConfig.experimental?.appDir && !!(findPagesDirResult && findPagesDirResult.appDir);
      ...
      cssRules.unshift({
        test: /vanilla-extract-virtual-file\.css/i,
      ...
      // prettier-ignore
      if (!config.plugins.some((p: any) => p instanceof NextMiniCssExtractPlugin)) {
      ...
      config.plugins.push(
        new VanillaExtractWebpackPlugin({ outputCss, ...pluginOptions }),
      );
